...
public boolean insertChiTuTatCaPhieuNhap() {
    try (Connection conn = DBConnection.getConnection()) {
        // 1. Truy vấn mã nhân viên có chức vụ "Quản lý kho"
        int maNvQuanLyKho = -1;
        String sqlGetMaNv = "SELECT ma_nv FROM nhanvien WHERE chucvu = 'kho' LIMIT 1";
        ...
        // 2. Lấy tổng chi phí nhập hàng theo từng ngày
        String sqlSelect = """
            SELECT 
        ...
        try (PreparedStatement stmtSelect = conn.prepareStatement(sqlSelect);
            ...
            while (rs.next()) {
                ...
                String sqlUpdate = """
                    UPDATE qltaichinh
                    SET so_tien = ?
                    WHERE loai_giao_dich = 'Chi' AND ngay = ?
                """;
                try (PreparedStatement stmtUpdate = conn.prepareStatement(sqlUpdate)) {
                    ...
                    if (rowsUpdated == 0) {
                        ...
                        try (PreparedStatement stmtInsert = conn.prepareStatement(sqlInsert)) {
                            ...
                            stmtInsert.setInt(4, maNvQuanLyKho); // chèn mã nhân viên thật
...
public boolean insertThuTuTatCaHoaDonVaDonHang() {
    try (Connection conn = DBConnection.getConnection()) {
        // 1. Lấy mã nhân viên có chức vụ duyệt đơn (cho đơn online)
        int maNvDuyetDon = -1;
        String sqlNv = "SELECT ma_nv FROM nhanvien WHERE chucvu = 'admin' LIMIT 1";
        try (PreparedStatement stmtNv = conn.prepareStatement(sqlNv);
             ResultSet rsNv = stmtNv.executeQuery()) {
            if (rsNv.next()) {
                maNvDuyetDon = rsNv.getInt("ma_nv");
            }
        }

        // 2. Lấy tổng thu từ hóa đơn bán offline
        String sqlHDOffline = """
            SELECT DATE(ngay_lap) AS ngay, SUM(tong_tien) AS tong_thu, ma_nv
            FROM hoa_don
            GROUP BY DATE(ngay_lap), ma_nv
        """;

        try (PreparedStatement stmtHD = conn.prepareStatement(sqlHDOffline);
            ...
            while (rsHD.next()) {
                ...
                String moTa = "Tổng thu từ hóa đơn ngày " + ngay;

                // Update trước
                String sqlUpdate = """
                    UPDATE qltaichinh
                    SET so_tien = ?, ma_nv = ?
                    WHERE loai_giao_dich = 'Thu' AND ngay = ?
                """;
                try (PreparedStatement stmtUpdate = conn.prepareStatement(sqlUpdate)) {
                    stmtUpdate.setFloat(1, tongThu);
                    stmtUpdate.setInt(2, maNv);
        ...
        // 3. Lấy tổng thu từ đơn đặt online (hdorder) — dùng chung maNvDuyetDon
        String sqlOrder = """
        ...
        try (PreparedStatement stmtOrder = conn.prepareStatement(sqlOrder);
            ...
            while (rsOrder.next()) {
                ...
                try (PreparedStatement stmtUpdate = conn.prepareStatement(sqlUpdate)) {
                    stmtUpdate.setFloat(1, tongThu);
                    stmtUpdate.setInt(2, maNvDuyetDon);
                    stmtUpdate.setDate(3, ngay);
                    int rowsUpdated = stmtUpdate.executeUpdate();
                    ...
                    if (rowsUpdated == 0) {
                        ...
                        try (PreparedStatement stmtInsert = conn.prepareStatement(sqlInsert)) {
                            ...
                            stmtInsert.setDate(2, ngay);
                            stmtInsert.setString(3, moTa);
                            stmtInsert.setInt(4, maNvDuyetDon);
                            stmtInsert.executeUpdate();
                        }
                    }
                }
